...
export default function DeleteProduct({id,title,type}:{id:string,title:string,type?:string}) {
  ...
  return (
    ...
    className='flex shadow-md items-center justify-center fixed 
    top-0 left-0 bottom-0 right-0 bg-gray-300/50'>
        <div onClick={(e) => e.stopPropagation()} className='w-fit dark:bg-gray-800/60 bg-white p-4 rounded-lg'>
            ...
            <div className='flex justify-around'>
                <button disabled={loading} onClick={()=>setConfirm(false)} 
                className='cursor-pointer transition hover:bg-blue-700 bg-blue-600 
                text-white px-2 py-1 rounded
                 disabled:bg-gray-700 disabled:cursor-not-allowed
                '>cancel</button>
                <button disabled={loading} onClick={()=>handleDelete()} 
                className='cursor-pointer transition hover:bg-red-700
                 bg-red-600 disabled:bg-gray-700 disabled:cursor-not-allowed
                 text-white px-2 py-1 rounded'>delete</button>
            </div>
        </div>
    </div>}
    </>

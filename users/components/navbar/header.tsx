...
`
const headerStyle="flex justify-between relative z-30 items-center p-3 bg-gradient-to-r from-purple-200 to-indigo-300"
export default function Header() {
  ...
  return (
    <header className={headerStyle}>
            ...
            <Link className={navStyle} href={"/category"}>Categories</Link>
            <Link className={navStyle + 'relative flex items-center'} href={"/cart"}>
              <ShoppingCart className="size-6" />
              <span className={`absolute -top-1 -right-2 ${cartIds.length===0?"bg-gray-500":"bg-red-600"} text-white text-xs rounded-full px-1`}>
                {cartIds.length}
              </span>
            </Link>
            <Link className={navStyle} href={"/account"}>Account</Link>
            </nav>
            ...
            className={navStyleMobile} href={"/"}>Home</Link>
            <Link 
            onClick={()=>setShowMobile(false)}
            className={navStyleMobile} href={"/AllProducts"}>All Products</Link>
            <Link 
            onClick={()=>setShowMobile(false)}
            className={navStyleMobile} href={"/category"}>Categories</Link>
            <Link 
            onClick={()=>setShowMobile(false)}
